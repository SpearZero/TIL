...
#### 원천 데이터
이벤트 소싱 패턴이 작동하려면 객체 상태에 대한 모든 변경 사항이 이벤트로 표현되고 저장되어야 한다. 이러한 이벤트는 시스템의 원천 데이터가 된다.

**이벤트 스토어(event store)**
- 이벤트를 저장하는 데 사용되는 데이터베이스
- 이벤트 추가만 가능하고 수정이나 삭제를 할 수 없다.
- 이벤트 소싱 패턴 구현을 위한 최소한의 기능
    - 특정 비즈니스 엔티티에 속한 모든 이벤트를 가져오기
    - 이벤트를 추가하기
        - 메서드에 낙관적 동시성 제어를 구현하기 위한 변수가 필요할 수도 있음
